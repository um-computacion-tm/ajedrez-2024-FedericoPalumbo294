...
class TestBoard(unittest.TestCase):
    ...
    def test_move_piece(self):
        # Mover una pieza y verificar que se haya movido correctamente
        self.assertTrue(self.board.move_piece((1, 0), (3, 0)))  # Mover peón negro
        self.assertIsInstance(self.board.get_piece(3, 0), Pawn)
        self.assertIsNone(self.board.get_piece(1, 0))

    def test_invalid_move(self):
        # Intentar mover una pieza de manera inválida y verificar que no se haya movido
        self.assertFalse(self.board.move_piece((0, 0), (0, 1)))  # Intentar mover torre negra de manera inválida
        self.assertIsInstance(self.board.get_piece(0, 0), Rook)
        self.assertIsInstance(self.board.get_piece(0, 1), Knight)  # Verificar que el caballo negro sigue en su lugar

    def test_display(self):
        # Verificar que el método display no genere errores
        self.board.display()
